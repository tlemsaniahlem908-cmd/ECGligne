"""
=============================================================
PTB-XL — MODEL B — MI vs STTC
Binary Classification avec Transfer Learning depuis Model A
Vertex AI / NVIDIA L4

🔥 TRANSFER LEARNING :
   Model A (NORM vs MI+STTC) → initialise ResNetHybrid pour Model B
   → fold1_resnet_hybrid_best.pt utilisé pour fold 1, etc.
   → convergence plus rapide + meilleure précision attendue

Architecture :
  ✅ ResNetHybrid  → Transfer Learning depuis Model A (fold-specific)
  ✅ InceptionHybrid → random init
  ✅ TCNHybrid       → random init
  ✅ ECG features (16) + clinical rules (10) + meta
  ✅ WeightedPolyLoss + label smoothing
  ✅ 5-fold OOF strict
  ✅ Pseudo-labeling sur OOF
  ✅ TTA×8
=============================================================
"""
...
# ── DONNÉES ──────────────────────────────────────────────────
DATA_DIR  = "/home/jupyter/ptbxl_processed_modelB_MI_vs_STTC"

# ── OUTPUT ───────────────────────────────────────────────────
OUTPUT_DIR = "/home/jupyter/model_binary_modelB_MI_vs_STTC"

# ── 🔥 TRANSFER LEARNING : chemin vers les checkpoints Model A ──
MODEL_A_DIR  = "/home/jupyter/model_binary_v10b_simclr_strong"

# ── SimCLR fallback si Model A non disponible ────────────────
...
def compute_metrics(y_true, probs, threshold):
    ...
    return {
        ...
        "f1_sttc"         : float(f1_score(y_true, y_pred, pos_label=1, zero_division=0)),
        "f1_mi"           : float(f1_score(y_true, y_pred, pos_label=0, zero_division=0)),
        "precision_sttc"  : float(precision_score(y_true, y_pred, pos_label=1, zero_division=0)),
        "precision_mi"    : float(precision_score(y_true, y_pred, pos_label=0, zero_division=0)),
        "sensitivity_sttc": float(tp / max(1, tp+fn)),
        "specificity_mi"  : float(tn / max(1, tn+fp)),
...
def plot_cm(cm, path, title):
    ...
    ax.set_xticklabels(["MI","STTC"])
    ax.set_yticklabels(["MI","STTC"])
    for i in range(2):
        for j in range(2):
...
# =============================================================
def load_data():
    print("="*60); print("LOADING DATA — MODEL B (MI vs STTC)"); print("="*60)
    ...
    print(f"Pool : {len(pool_idx)} | MI={(y_pool==0).sum()} | STTC={(y_pool==1).sum()}")
    print(f"Test : {len(test_idx)} | MI={(y_test==0).sum()} | STTC={(y_test==1).sum()}")
...
class ResNetHybrid(nn.Module):
    """
    ResNet + Transfer Learning depuis Model A + meta/feat/rule.
    🔥 TRANSFER LEARNING : load_simclr_encoder() charge les poids
       de Model A (fold-specific) au lieu du SimCLR générique.
    """
    ...
    def load_simclr_encoder(self, path):
        """
        Charge les poids depuis un fichier .pt
        Fonctionne pour SimCLR ET pour Model A checkpoint.
        Seuls stem + layers sont chargés (pas le head).
        """
...
def train_one(name, model, X,
              ...
              fold=1):   # ← fold nécessaire pour le transfer learning
    print(f"\n{'='*60}"); print(f"TRAIN: {name}"); print(f"{'='*60}")
    model = model.to(DEVICE)
    print(f"  Params: {sum(p.numel() for p in model.parameters()):,}")

    # =========================================================
    # 🔥 TRANSFER LEARNING ICI
    # Priority 1 : Model A checkpoint (fold-specific)
    # Priority 2 : SimCLR encoder (fallback)
    # =========================================================
    if use_simclr and hasattr(model, "load_simclr_encoder"):
        model_a_path = os.path.join(MODEL_A_DIR,
                                    f"fold{fold}_resnet_hybrid_best.pt")
        if os.path.exists(model_a_path):
            print(f"  🔥 TRANSFER LEARNING depuis Model A:")
            print(f"     {model_a_path}")
            model.load_simclr_encoder(model_a_path)
        elif os.path.exists(SIMCLR_PATH):
            print(f"  ⚠️  Model A non trouvé → fallback SimCLR")
            print(f"     {SIMCLR_PATH}")
            model.load_simclr_encoder(SIMCLR_PATH)
        else:
            print(f"  ⚠️  Ni Model A ni SimCLR → training from scratch")
            use_simclr = False
...
def run_pseudo_labeling(
    ...
    print(f"  MI pseudo     : {(yps==0).sum()}")
    print(f"  STTC pseudo   : {(yps==1).sum()}")
...
def main():
    ...
    print(f"MODEL_A_DIR  : {MODEL_A_DIR}")
    ...
    MODEL_BUILDERS = {
        "resnet_hybrid"   : (ResNetHybrid,    True),   # True = use Transfer Learning
    ...
    for fold, (tr_local, va_local) in enumerate(
        ...
        print(f"Train: {len(idx_tr)} | MI={(y_tr==0).sum()} | STTC={(y_tr==1).sum()}")
        print(f"Val  : {len(idx_va)} | MI={(y_va==0).sum()} | STTC={(y_va==1).sum()}")
        ...
        for key, (builder, use_simclr) in MODEL_BUILDERS.items():
            ...
            print(f"\n{name.upper()} — {sum(p.numel() for p in model.parameters()):,} params"
                  + (" [Transfer from Model A]" if use_simclr else ""))
    ...
    print(f"\n{'='*70}")
    print("FINAL MODEL B — MI vs STTC — RESULT")
    print(f"{'='*70}")
    ...
    print(classification_report(y_test.astype(int), test_pred.astype(int),
                                target_names=["MI","STTC"], digits=4))
    print("\nTEST CONFUSION MATRIX:"); print(test_cm)

    save_json({
        "config": {
            "model": "Model B — MI vs STTC",
            "transfer_learning": "Model A fold-specific checkpoints",
            "model_a_dir": MODEL_A_DIR,
            "n_folds": N_FOLDS, "use_three_models": USE_THREE_MODELS,
            "use_pseudo_labeling": USE_PSEUDO_LABELING,
            "epochs": EPOCHS, "patience": PATIENCE,
            "batch": BATCH, "lr_head": LR_HEAD,
            "lr_encoder": LR_ENCODER, "tta_n": TTA_N
        },
        "fold_logs": logs,
        ...
        "oof_metrics":  oof_m,
        "test_metrics": test_m
    }, pth("metrics_modelB_MI_vs_STTC.json"))

    np.save(pth("oof_probs.npy"),   final_oof)
    np.save(pth("test_probs.npy"),  final_test)
    np.save(pth("test_pred.npy"),   test_pred)
    plot_cm(oof_cm,  pth("confusion_matrix_oof.png"),  "OOF Confusion Matrix — MI vs STTC")
    plot_cm(test_cm, pth("confusion_matrix_test.png"), "Test Confusion Matrix — MI vs STTC")
